...
def generate_single_audio(name_id, text, status, output_name, config, audio_directory):  # Added config and audio_directory
    ...
    path = config.get('SOVITS', f'path{name_id}')
    prompt_text = config.get('SOVITS', f'text{name_id}')
    if status == 0:
        requests.get(
    ...
    Lang = "zh"
    Theme_Language = config.get('剧情', 'Language') #Read from the config at the time it's used
...
def getvoice(start_id=None):
    ...
    story_file = os.path.join(game_directory, "data", story_title, "story.json")
    ...
    for i in range(start_index, len(conversations)):
        ...
        model_name = config.get('SOVITS', f'model{place_id}')  # Get model_name here
...
def getendvoice(start_id=1):
    ...
    story_file = os.path.join(game_directory, "data", story_title, "end.json")
    character_file = os.path.join(game_directory, "data", story_title, "character.json")
    ...
    for i in range(start_index, len(conversations)):
        ...
        # Check if the audio file already exists
        output_file_path = os.path.join(audio_directory, f"end_{conversation_id}.wav")
        ...
        generate_single_audio(place_id, text_replaced, status=status, output_name="end_"+str(conversation_id), config=config,
                              audio_directory=audio_directory)
